...
class Visualiser():
    ...
    def draw_grid(self):
        radar_filter = pygame.Surface((self.width, self.height), pygame.SRCALPHA)
        radar_filter.fill((30, 30, 30, 200))
        self.screen.blit(radar_filter, (0, 0))

        self.cell_size = min(self.width // (self.dif_x), self.height // (self.dif_y))
        ...
        pygame.draw.circle(self.screen, (255, 0, 0), (self.origin_x, self.origin_y), 5)

    def draw_connections(self):
        for c in self.map.connections:
            mx = self.origin_x + c[0][0] * self.cell_size
            my = self.origin_y + c[0][1] * self.cell_size
            gx = self.origin_x + c[1][0] * self.cell_size
            gy = self.origin_y + c[1][1] * self.cell_size
    ...
    def color_hub(self, hub: dict) -> tuple[int, int, int]:
        if Color(hub["metadata"].color) == Color.rainbow:
        ...
        return colors[Color(hub["metadata"].color)]
    ...
    def draw_assets(self):
        ...
        try:
            for h in self.all_hubs:
                mode = zone[Zone(h['metadata'].zone)] + ".png"
            ...
            raise FileNotFoundError(f"Asset zone type not found")
